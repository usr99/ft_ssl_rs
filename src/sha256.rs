...
pub fn hash(message: &[u8]) -> Hash {
	...
	for i in (0..bytes).step_by(BLOCK_SIZE) {
		...
		for j in 0..BLOCK_SIZE {
			...
			let ch = (e & f) ^ (!e & g);
			let tmp1 = h.wrapping_add(s1).wrapping_add(ch).wrapping_add(K[j]).wrapping_add(W[j]);
...
mod tests {
	...
	#[test_case("a5482539287a4069ebd3eb45a13a47b1968316c442a7e69bc6b9c100b101d65d", "42 is nice\n")]
